...
class ProfileController extends Controller
{
    ...
    public function user_profile_update(Request $request)
    {
        ...
        $changes = [];
        foreach ($request->only(['name', 'email', 'phone_1', 'phone_2', 'address', 'age', 'dob', 'gender', 'marital_status']) as $field => $value) {
            if ($user->{$field} != $value) {
    ...
    public function profileManagement()
    {
        ...
        return view('profile_management', compact('users'));
    }

    public function userLog()
    {
        $logs = UserActivityLog::with('user')->latest()->paginate(10);
        return view('user_activity_log', compact('logs'));
    }
}
